...
### ウォーターフォール(浮動バー・増減要因の分解)

前期→当期の変化を要因ごとに積み上げる図の定型(実例: slide-waterfall)。開始/終了は0から伸びる「合計バー」、途中は前の値から浮く「増減バー」。

```
baseY = 下端のy座標、plotH = 描画高さ、maxV = 最終値より少し大きいキリの良い値
y(v) = baseY - v / maxV * plotH   ← 各「累計値」のy座標を先に全部計算する
合計バー(開始・終了): x, width固定。y = y(その値)、height = baseY - y(その値)
増加バー: 前累計→後累計。y = y(後累計)(高い方)、height = y(前累計) - y(後累計)
減少バー: 前累計→後累計(値が下がる)。y = y(後累計の方が大きいy、つまり低い値側の上端)にはならず、
          y = y(高い方の値)、height = y(低い方の値) - y(高い方の値) ← 増加バーと同じ式で機械的に求まる
```
- 隣接バーは「前バーの終端の高さ」で必ず接続する。バー間に**破線の接続線**(水平)を引くと繋がりが伝わる
- 色は2色まで: 増加=アクセント(緑等)、減少=アンバー`#d97706`(design-guideの診断系配色の例外を適用し、ヘッダーコメントに明記する)。合計バーは中間色(`--sub`相当)にして増減バーと区別する
- 全ステップを機械的に計算してから描く。暗算で高さを決めない(実例のコメントに計算式を残す)
